...
def get_db_products():
    ...
    for row in rows:
        ...
        for key, default in [("colorImages", {}), ("specs", []), ("colorQuantities", {}), ("gallery", [])]:
            db_key = "color_images" if key == "colorImages" else ("color_quantities" if key == "colorQuantities" else key)
            try:
...
async def cmd_start(message: Message):
    shop_reply_keyboard = ReplyKeyboardMarkup(
        keyboard=[[KeyboardButton(text="🛍 Відкрити M1lipStore", web_app=WebAppInfo(url="https://wincher1202.github.io/M1lipStore-/"))]],
...
async def process_manage_product(callback: CallbackQuery):
    ...
    try:
        if product.get('img'):
            await callback.message.answer_photo(photo=product['img'], caption=caption, parse_mode="Markdown", reply_markup=action_markup)
        else:
...
async def save_edited_title(message: Message, state: FSMContext):
    ...
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE products SET title = %s WHERE id = %s", (message.text.strip(), data.get("editing_product_id")))
    conn.commit()
    cursor.close()
    conn.close()
...
async def process_edit_colors(callback: CallbackQuery):
    ...
    cq = product.get('colorQuantities', {})
    buttons = [[InlineKeyboardButton(text=f"Змінити залишок: {color} ({cq[color]} шт.)", callback_data=f"ch_cqty_{product_id}_{color}")] for color in cq.keys()]
...
async def save_color_qty_edit(message: Message, state: FSMContext):
    ...
    try:
        ...
        if product:
            ...
            cursor.close()
            conn.close()
            await message.answer("✅ Залишок оновлено! Напишіть /admin.")
            await state.clear()
    except ValueError:
...
async def ask_category_step(message: Message, state: FSMContext):
    ...
    await message.answer("📁 Оберіть категорію:", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🖱 Миші", callback_data="cat_Миші"), InlineKeyboardButton(text="⌨️ Клавіатури", callback_data="cat_Клавіатури")],
        [InlineKeyboardButton(text="🎧 Гарнітури", callback_data="cat_Гарнітури"), InlineKeyboardButton(text="🔌 Аксесуари", callback_data="cat_Аксесуари")]
    ]))
...
async def select_category_cb(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        pass
    await callback.message.answer("⚙️ Введіть характеристики (наприклад: `Сенсор (PAW3349)`). Коли закінчите, надішліть `/done`:")
    await callback.answer()
...
async def process_spec_input(message: Message, state: FSMContext):
    ...
    if message.text.strip().lower() == '/done':
        ...
        await message.answer("🎨 Оберіть кольори:", reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🖤 Чорний", callback_data="col_Чорний"), InlineKeyboardButton(text="🤍 Білий", callback_data="col_Білий")],
    ...
    label, value = (text.split("(", 1)[0].strip().upper(), text.split("(", 1)[1].replace(")", "").strip()) if "(" in text and ")" in text else (f"БЛОК {data.get('current_spec_index', 1)}", text)
...
async def process_color_qty_input(message: Message, state: FSMContext):
    ...
    try:
        ...
        data = await state.get_data()
        colors, idx, cq = data.get("selected_colors"), data.get("current_color_qty_index"), data.get("color_quantities_dict", {})
        cq[colors[idx]] = qty
        idx += 1
        if idx < len(colors):
            ...
            await message.answer(f"🖼 Надішліть головне фото для кольору **{colors[0]}**:", reply_markup=InlineKeyboardMarkup(
...
async def skip_col_main_photo_cb(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        pass
    await callback.message.answer("📸 Надішліть додаткові фото для цього кольору (або натисніть кнопку):", reply_markup=InlineKeyboardMarkup(
...
async def process_color_main_photo(message: Message, state: FSMContext):
    if message.from_user.id not in ADMIN_IDS: return
    data = await state.get_data()
    colors, idx, c_dict = data.get("selected_colors"), data.get("current_color_photo_index"), data.get("color_images_dict", {})
...
async def process_color_gallery_photo(message: Message, state: FSMContext):
    ...
    await state.update_data(color_images_dict=c_dict)
    await message.answer(f"✅ Додано в галерею ({len(c_dict[colors[idx]]['gallery'])}). Надішліть ще або натисніть «Готово».")
...
async def col_gallery_done_cb(callback: CallbackQuery, state: FSMContext):
    ...
    if idx < len(colors):
        ...
        await state.set_state(AddProductStates.waiting_for_color_main_photo)
        await callback.message.answer(f"🖼 Надішліть головне фото для кольору **{colors[idx]}**:", reply_markup=InlineKeyboardMarkup(
            inline_keyboard=[[InlineKeyboardButton(text="⏭ Пропустити", callback_data="skip_col_main_photo")]]))
    else:
...
async def add_gen_gal(message: Message, state: FSMContext):
    ...
    await message.answer(f"✅ Додано ({len(g)}). Надішліть ще або завершіть.")
...
async def save_product_to_db(message: Message, state: FSMContext):
    ...
    """, (
        ...
        data.get("tag", ""),
        data.get("category"),
        sum(data.get("color_quantities_dict", {}).values()),
        ", ".join(data.get("selected_colors", [])),
