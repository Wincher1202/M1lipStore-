...
try:
    ...
    class DeliveryProviderNotConfigured(Exception): pass
    class DeliveryProviderError(Exception): pass
...
def init_db():
    if not DATABASE_URL:
        logging.warning("DATABASE_URL is not set. Database initialization skipped.")
        return
    try:
        ...
        logging.error(f"Error during init_db: {e}")
...
async def admin_get_order(order_id: str, request: Request):
    require_admin(request)
    if not DATABASE_URL: raise HTTPException(status_code=500, detail="Database not configured")
    conn = get_db_connection()
    cursor = conn.cursor()
...
def get_active_categories():
    if not DATABASE_URL: return ["Миші", "Клавіатури", "Гарнітури", "Аксесуари"]
    conn = get_db_connection()
    cursor = conn.cursor()
